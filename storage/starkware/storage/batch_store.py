...
class BatchStore(Storage):
    def __init__(self, storage: Storage, n_workers_set: int, n_workers_get: int):
        ...
        self.tasks = [asyncio.create_task(self.set_value_thread())
                      for _ in range(n_workers_set)]
        self.tasks += [asyncio.create_task(self.get_value_thread())
                       for _ in range(n_workers_get)]
        ...
        for task in self.tasks:
            ...
            try:
                ...
                if not isinstance(ex, asyncio.CancelledError):
                    logger.error(f'Excpetion occurred!', exc_info=True)
